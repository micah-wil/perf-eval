...
- `PERF_EVAL_RUN_TYPE` — free-form categorization marker stamped onto every ingested row as `run_type` (e.g. `nightly`, `pr`, `rc`, `aiter_nightly`). The dashboard groups and compares builds by this value, so new categories can be added by just passing a new string — no code change required. Defaults to `adhoc` when unset. Set it at the pipeline/schedule level so it applies to every step. Purely a label — it does **not** trigger the nightly behavior (use `NIGHTLY` for that).
