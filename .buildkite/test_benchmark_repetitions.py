...
def test_all_h200_benchmarks_use_saturated_warmups_and_three_runs():
    ...
    # This branch carries AMD-only workloads (no H200/B200 recipes), so an
    # empty result here is expected rather than a regression.
    for path in paths:
        with open(path) as f:
...
def test_long_h200_workloads_have_expected_timeouts():
    ...
    # This branch carries AMD-only workloads, so these H200 recipes are
    # intentionally absent; skip rather than fail when they're missing.
...
def test_gpt_oss_b200_fixed_length_benchmark_uses_raw_completions():
    ...
    if not os.path.exists(path):
        # This branch carries AMD-only workloads; no B200 recipe to check.
        return
    with open(path) as f:
        workload = yaml.safe_load(f)
